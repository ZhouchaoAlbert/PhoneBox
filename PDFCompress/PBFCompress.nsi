...
VIAddVersionKey "CompanyName"       "深圳市智城时代科技有限公司"
VIAddVersionKey "FileVersion"       "${PRODUCT_VERSION}"
VIAddVersionKey "InternalName"      "PDFReduce.exe"
VIAddVersionKey "FileDescription"   "${PRODUCT_NAME}"
VIAddVersionKey "LegalCopyright"    "版权所有（c）2016 智城时代"
...
; 输出的安装包名
OutFile   "YueShuPDFTool_Install_${PRODUCT_VERSION}.exe"  
...
; 安装和卸载页面
Page         custom     YueShuPDFToolUI
UninstPage   custom     un.YueShuPDFToolUI
...
Function .onInit
    ...
    #初始化MsgBox窗口
    UISkin::InitMessageBox "MessageBox.xml" "label_box_title" "text_box_tip" "btn_box_close" "btn_box_yes" "btn_box_no" "setup2.ico"
    Pop $MessageBoxHandle 
	
	#检测程序是否正在运行
    System::Call 'kernel32::CreateMutexW(i 0, i 0, t "悦书PDF工具") i .r1 ?e'
    Pop $R1	 ;安装程序已经运行
    StrCmp $R1 0 AA BB
	BB: 	;弹窗提示
	 UISkin::ShowMessageBox "提示" "悦书PDF工具 安装包程序已经在运行！" "确定" "取消"
...
Function YueShuPDFToolUI
    ;初始化窗口  
	InitPluginsDir   	
	SetOutPath "$PLUGINSDIR"
	File /r "imageres\*"
    ;安装主界面
	UISkin::ShowInstallSkin "install.xml" "setup.ico2" "悦书PDF工具"
	Pop $Dialog	  
    ;设置焦点
...
Function OnInstallBtnFunc
   ...
   ;安装前更新下磁盘
    ...
    nsProcessW::_FindProcess PDFReduce.exe"
    Pop $R0	
    ${If} $R0 == 0
        UISkin::ShowMessageBox "提示" "您的 悦书PDF工具 正在运行,是否关闭开始安装?" "开始安装" "取消"
        Pop $R1 
        ${If} $R1 == 1
          Call KillAllProc
        ${EndIf}
...
Function OnCloseBtnFunc
	UISkin::ShowMessageBox "提示" "您确定要退出 悦书PDF工具 安装程序吗?" "确定" "取消"
	Pop $R1 
    ${If} $R1 == 1
      UISkin::ExitSkinEngine
    ${EndIf}
...
Function BuildShortCut 
   ...
   CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT_NAME}\${PRODUCT_SHORTCUT_NAME}.lnk" "$INSTDIR\Bin\PDFReduce.exe" ""
   ...
   ;桌面快捷方式	
   SetShellVarContext all
   CreateShortCut "$DESKTOP\${PRODUCT_SHORTCUT_NAME}.lnk" "$INSTDIR\Bin\PDFReduce.exe" ""   
   SetShellVarContext current
...
!endif
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}" "DisplayIcon" "$INSTDIR\Bin\PDFReduce.exe"
...
!endif
  ...
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}" "NoRepair" "1"
   ...
   SetShellVarContext all
	 DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "YueShuPDFTool"
   SetShellVarContext current
...
Function un.onInit	  
   ...
   ;初始化MsgBox窗口
    ...
    StrCmp $R1 0 AA BB
	BB: ;弹窗提示
	 UISkin::ShowMessageBox "提示" "悦书PDF工具 卸载包程序已经在运行！" "确定" "取消"
	 UISkin::ExitSkinEngine
    Abort
AA:
...
Function un.YueShuPDFToolUI
   ...
   ;初始化窗口  
    ...
    ;卸载主界面
	UISkin::ShowInstallSkin "install.xml" "setup2.ico" "悦书PDF工具"
	Pop $Dialog	  
...
Function un.onUninstall 
   ...
   ;到卸载页 判断EC是否在运行
	...
	Pop $R0
  ${If} $R0 == 0
      UISkin::ShowMessageBox "提示" "您的 悦书PDF工具 正在运行,是否关闭立即卸载?" "立即卸载" "以后再说"
      Pop  $R1
      ${If} $R1 == 1  ;立即卸载
           Call un.KillAllProc  
      ${Else} ;关闭
...
AA:
    NsisPro::KillProcess "PDFReduce.exe"
    Sleep 100
    NsisPro::FindProcess "PDFReduce.exe"
    Pop $R0	
    StrCmp $R0 "1" BB CC
    BB:
      UISkin::ShowMessageBox "提示" "悦书PDF工具 进程正在运行,请手动关闭后点击“重试”" "重试" "取消"
      Pop  $R1
      ${If} $R1 == 1  ; 重试
       Sleep 300
        goto AA
