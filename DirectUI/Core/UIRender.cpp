...
void CRenderEngine::DrawRoundRect(HDC hDC, const RECT& rc, int nSize, int width, int height, DWORD dwPenColor)
{
 ...
 void CRenderEngine::DrawRoundRect2(HDC hDC, const RECT& rc, int width, int height, int nSize, DWORD dwPenColor)
 {
	 //GDI+  绘制的边框不圆滑很难看改用GDI+
	 Gdiplus::Graphics g(hDC);
	 g.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
	 //创建一个红色的画笔  
	 Gdiplus::Pen *pen = new Gdiplus::Pen(Gdiplus::Color(255, GetRValue(dwPenColor), GetGValue(dwPenColor), GetBValue(dwPenColor)), nSize);

	 RECT rc_ = rc;
	 rc_.right -= 1;
	 rc_.bottom -= 1;
	 Gdiplus::GraphicsPath path;
	 path.AddArc(rc_.left, rc_.top, 2 * width, 2 * height, 180, 90);
	 path.AddLine(rc_.left + width, rc_.top, rc_.right - width, rc_.top);

	 path.AddArc(rc_.right - 2 * width, rc_.top, 2 * width, 2 * height, 270, 90);
	 path.AddLine(rc_.right, rc_.top + height, rc_.right, rc_.bottom - height);


	 path.AddArc(rc_.right - 2 * width, rc_.bottom - 2 * height, 2 * width, 2 * height, 0, 90);
	 path.AddLine(rc_.right - width, rc_.bottom, rc_.left + width, rc_.bottom);

	 path.AddArc(rc_.left, rc_.bottom - 2 * width, 2 * width, 2 * height, 90, 90);
	 path.AddLine(rc_.left, rc_.top + height, rc_.left, rc_.bottom - height);
	 g.DrawPath(pen, &path);
	 delete pen;
 }
